...
export class UpdateProductAdminComponent implements OnInit {
 ...
 isCategorySelected(categoryId: number): boolean {
  ...
  updateProduct() {
    ...
    else if (this.updatedEbook.kindofbook === KindOfBook.FOR_SALE && this.updatedEbook.price <0) {
      alert('Tiền không là số âm')
      // Giữ nguyên giá trị price nếu là FOR_SALE
      this.updatedEbook.price = this.updatedEbook.price; // Điều này là không cần thiết nhưng giữ lại để rõ ràng
    }
    ...
    const updateEbookDTO: UpdateEbookDTO = {
      ...
      category_id: this.updatedEbook.category_id
    };
